...
int change_html_para(char * str,char *str1,char *str2,char *str3)
{
	char tmp[100];
	if(check_change_line(str,"var ID_V =",str1)==0)
	return 0;

	if(check_change_line(str,"var random_number_V =",str2)==0)
	return 0;

	if(check_change_line(str,"var correlation_gamma_V =",str3)==0)
	return 0;
...
int get_param(char *str1,char *str2,char *str3)
{
	int res;
	unsigned int size;
//	char *query = NULL;
	...
	//如果函数读取成功，则返回指针temp，失败则返回 NULL。                            //fgetc能够实现fp自加
		if (ch == NULL)
			break;
		change_html_para(temp,str1,str2,str3);//将参数写入缓存中的bc_para_set.htlm
	}
	fclose( fp );
   return 0;
}
int main(int argc,char * argv[])
{
    ...
    get_param(param1,param2,param3);
    printf("param1=%s\n",param1);
    printf("param2=%s\n",param2);
    printf("param3=%s\n",param3);
}